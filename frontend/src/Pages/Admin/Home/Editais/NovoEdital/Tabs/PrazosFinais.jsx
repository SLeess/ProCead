...
export default function PrazosFinais({formData, handleOnChangeAttr, setFormData}){
    const handleDeleteMoment = (indexToDelete) => {
        setFormData(f => ({
            ...f,
            momentosDeRecursos: f.momentosDeRecursos.filter((_, index) => index !== indexToDelete)
        }));
    };
    ...
    const handleAddMomentoRecurso = () => {
        if (newMomentoRecurso.trim()) {
            ...
            setFormData(f => ({
                ...f,
                momentosDeRecursos: [...f.momentosDeRecursos, newMoment]
            }));
    ...
    const handleChangeMomentoRecurso = (e, attr, index) => {
        ...
        const novosMomentos = formData.momentosDeRecursos.map((item, idx) => {
            if (idx !== index) {
                return item;
            }
        ...
        setFormData(f => ({
            ...f,
            momentosDeRecursos: novosMomentos,
        }));
    };
                ...
                <div className="grid grid-cols-12 gap-x-8 gap-y-6">
                    ...
                    <div className="relative overflow-x-auto shadow-md sm:rounded-lg col-span-12">
                        <table className="w-full text-sm text-left text-gray-500">
                            ...
                            <tbody>
                                {formData?.momentosDeRecursos.length > 0 ? (
                                    formData.momentosDeRecursos.map((moment, index) => (
                                        <tr key={index} className="bg-white border-b hover:bg-gray-50">
                                            ...
                                            <td scope="row" className="px-6 py-4 font-medium text-gray-900">
                                                <DateTimePicker 
                                                    ...
                                                    value={formData.momentosDeRecursos[index].start} 
                                            ...
                                            <td scope="row" className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                                                <DateTimePicker 
                                                    id={"inicio_avaliacao_socioeconomico"}
                                                    value={formData.momentosDeRecursos[index].end} 
