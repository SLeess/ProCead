...
export default function ModalExportarRelatorio({ openModal, onCloseModal, table, title, selectedRowsData}) {
  ...
  const handleExport = async (titulo, subtitulo, orientacao, formato) => {
    ...
    const resultRows = Object.values(selectedRowsData);
    ...
    const rows = resultRows.map(row => {
      ...
      table.getVisibleLeafColumns().forEach(column => {
        if (column.id in row) {
          filteredRowData[column.id] = row[column.id];
        }
      });
